...
class SnorkelInsuranceEnv(Environment):
    ...
    def __init__(
        ...
        # Explicit task-id selection, overriding the fractional split. Required for the
        # Act-PRM split: train/eval/rollout are partitioned by QUESTION upstream, and the
        # gym must evaluate exactly the 41 held-out rollout questions -- not a fraction of
        # its own 300-task set. On finance an undeclared eval_query_ids key meant the flag
        # was silently dropped and all ten rollout runs scored the wrong 11 questions.
        train_task_ids: list | None = None,
        eval_task_ids: list | None = None,
        frac_train: float = 0.6,
        frac_val: float = 0.2,
        frac_test: float = 0.2,
        eval_all_samples: bool = False,
        max_turns: int = 50,
        num_tries: int = 1,
        seed: int = 0,
        split: str = "train",
        ...
        **kwargs: Any,
        ...
        self.train_task_ids = [str(i) for i in train_task_ids] if train_task_ids is not None else None
        self.eval_task_ids = [str(i) for i in eval_task_ids] if eval_task_ids is not None else None
        self.frac_train = frac_train
        self.frac_val = frac_val
        self.frac_test = frac_test
        # When True, every split (train/eval/test) holds the FULL dataset so an
        # eval run can score all samples (treat all samples as the eval set).
        self.eval_all_samples = eval_all_samples
        self.split = split
        ...
        # Initialize tools
        self.tool_registry: dict[str, BaseTool] = {name: cls() for name, cls in TOOL_CLASSES.items()}
        self.tool_descriptions = [tool.get_tool_desc() for tool in self.tool_registry.values()]
    ...
    def init_data(self) -> DatasetDict:
        ...
        for task_entry in tasks:
            ...
            building_construction = str(user_info.get("building_construction", ""))
            total_insured_value_property = str(user_info.get("total_insured_value_property", ""))
    ...
    def _build_reference_answer(self, task_type: str, user_info: dict[str, Any]) -> str:
        ...
        if task_type == "appetite_determination":
            ...
            return f"The appetite for {lob} is: {appetite}."
            ...
            return f"Recommended LOBs based on company profile. Primary LOB ({lob}): appetite is {appetite}."
            ...
            lob = user_info.get("lob", "")
            return f"Recommended policy limits for {lob}: ${limits_per}M per occurrence / ${limits_agg}M aggregate."
        elif task_type == "deductible_recommendation":
            deductible = user_info.get("deductible", "")
            lob = user_info.get("lob", "")
            return f"Recommended deductible for {lob}: ${deductible}."
            ...
            return (
                f"Small business eligibility: {is_small}. Revenue: {annual_revenue}, Employees: {number_of_employees}."
            )
    ...
    def _get_splits(self, dataset: Dataset) -> DatasetDict:
        ...
        if self.train_task_ids is not None or self.eval_task_ids is not None:
            tr = set(self.train_task_ids or [])
            ev = set(self.eval_task_ids or [])
            qids = [str(q) for q in dataset["query_id"]]
            idx_tr = [i for i, q in enumerate(qids) if q in tr]
            idx_ev = [i for i, q in enumerate(qids) if q in ev]
            missing = (tr | ev) - set(qids)
            if missing:
                logger.warning(
                    "snorkel_insurance: %d requested task_ids not in the task set: %s",
            ...
            logger.info("snorkel_insurance (explicit ids): train=%d eval=%d", len(d_tr), len(d_ev))

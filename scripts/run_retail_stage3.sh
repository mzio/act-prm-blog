...
#
# NOTE: the tau2-gym RL path is UNTESTED end-to-end here, hence the smoke gate. RL uses
# tau2's own task split (the rl_eval->tau2-index map is still a TODO) — flagged in logs.
#
...
for v in actions_only expert_thoughts thoughts_policy thoughts_base thoughts_policy_last thoughts_base_last; do
  ...
  log "RL $v from $ck ..."
  wait_gpu_free
  ./scripts/train_rl_from_sft.sh retail "$ck" --run_tag "$tag" > "$MDIR/s3_${v}.log" 2>&1 \

...
G=/tmp/aprm/expert_all; mkdir -p "$G"
LOG="$G/chain.log"
...
for step in step_0020 step_best; do
  ...
  CKPT_TAG="sgdlr1e_3" ./scripts/run_sft_rollout_eval.sh >>"$LOG" 2>&1
  log "  rc=$?"; touch "$m"; reap
done
...
for step in step_0020 step_best; do
  ...
  CKPT_TAG="sgdlr1e_3" ./scripts/run_finance_rollout.sh >>"$LOG" 2>&1
  log "  rc=$?"; touch "$m"; reap
done
...
if [ -f /tmp/aprm/insurance_rollout/SMOKE_OK ]; then
  for step in step_0020 step_best; do
    ...
    CKPT_TAG="sgdlr1e_3" ./scripts/run_insurance_rollout.sh >>"$LOG" 2>&1
    log "  rc=$?"; touch "$m"; reap
  done
else

#!/usr/bin/env bash
# expert_thoughts_all at SGD 1e-3 / sft_flat, across all four domains + rollouts.
#
...
LRARG="${1:?lr}"; LRTAG="lr${LRARG//[-.]/_}"
G=/tmp/aprm/expert_all_${LRTAG}; mkdir -p "$G"
LOG="$G/chain.log"
...
train_one(){  # $1=envcfg  $2=short  $3=EXPERT_POOL ("" = use default)
  ...
  env ${pool:+EXPERT_POOL=$pool} \
    TRAINER_CFG=sft_flat LR="$LRARG" OPTIMIZER=sgd NUM_BATCHES=200 EVAL_EVERY=5 \
...
for step in step_0020 step_best; do
  ...
  CKPT_TAG="sgd${LRTAG}" ./scripts/run_sft_rollout_eval.sh >>"$LOG" 2>&1
  log "  rc=$?"; touch "$m"; reap
done
...
for step in step_0020 step_best; do
  ...
  CKPT_TAG="sgd${LRTAG}" ./scripts/run_finance_rollout.sh >>"$LOG" 2>&1
  log "  rc=$?"; touch "$m"; reap
done
...
if [ -f /tmp/aprm/insurance_rollout/SMOKE_OK ]; then
  for step in step_0020 step_best; do
    ...
    CKPT_TAG="sgd${LRTAG}" ./scripts/run_insurance_rollout.sh >>"$LOG" 2>&1
    log "  rc=$?"; touch "$m"; reap
  done
else

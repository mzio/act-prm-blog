...
LENGTH_PENALTY = 0.15  # legacy default; real value is read per-run from config.json


def _run_hparams(gen_file: Path) -> dict[str, Any]:
    """Hyperparameters of the run that produced ``gen_file``, from its config.json.

    Previously LENGTH_PENALTY was a module constant of 0.15 and got stamped into the
    published reward column and dataset card. The 08-28 AdamW runs use
    length_penalty=0 with advantage_mode=action_probs, so a constant would have
    mislabelled the data with a reward formula that was never applied. Read it.
    """
    cfg_p = gen_file.parent / "config.json"
    cfg = json.loads(cfg_p.read_text()) if cfg_p.is_file() else {}

    # The relabel pass runs --no_train, so ITS optimizer/lr are meaningless (they sit at
    # their defaults -- "sgd" -- because the driver only passes --optimizer to the EM
    # step). The hyperparameters that actually shaped these thoughts belong to the EM run
    # whose checkpoint the relabel resumed from. Publishing the relabel's "sgd" would have
    # asserted the exact opposite of how the model was trained, so follow resume_from back
    # to the EM run's own config.json.
    em: dict[str, Any] = {}
    resume = cfg.get("resume_from")
    if resume:
        em_run = Path(str(resume))
        while em_run.name and not em_run.name.startswith(("step_", "checkpoint")):
            break
        em_dir = em_run.parent if em_run.name.startswith(("step_", "checkpoint")) else em_run
        em_cfg = REPO_ROOT / "logs" / Path(*em_dir.parts[-3:]) / "config.json"
        if em_cfg.is_file():
            e = json.loads(em_cfg.read_text())
            em = {
                "run_dir": em_dir.name,
                "optimizer": e.get("optimizer"),
                "learning_rate": e.get("learning_rate"),
                "num_batches": e.get("num_batches"),
                "advantage_mode": e.get("advantage_mode"),
                "length_penalty": e.get("length_penalty"),
                "lora_config": e.get("lora_config"),
            }
    return {
        # scoring pass: these describe the published reward/likelihood columns
        "length_penalty": cfg.get("length_penalty", LENGTH_PENALTY),
        "group_size": cfg.get("group_size"),
        "advantage_mode": cfg.get("advantage_mode"),
        "lora_config": cfg.get("lora_config"),
        "run_dir": gen_file.parent.name,
        # training pass: how the scoring LoRA itself was produced
        "em": em or None,
    }
...
- %%SUITE%%
- %%DOMAIN%%
pretty_name: Act-PRM SFT thoughts (%%SUITE%% %%DOMAIN%%)
configs:
- config_name: default
  data_files:
%%SPLIT_CONFIG%%
...
# Act-PRM SFT thoughts — %%SUITE%% %%DOMAIN%%
...
concatenate the files and filter.
...
Generated by Act-PRM Stage-1 relabel passes over %%SUITE%% %%DOMAIN%% expert
...
The LoRA that scored `p(x | s, z)` was itself trained by the Stage-1 EM:
...
def main() -> None:
    ...
    ap.add_argument(
        "--variants",
        default="",
        help="Comma-separated 'split_name=run_tag' pairs naming the relabel passes to "
        "publish, e.g. 'policy_best=retail_s1relabel_policy_adamw30'. Default (empty) "
        "reproduces the legacy 2-scorer x 2-checkpoint set of "
        "'<prefix>_<scorer>_<ckpt>_heldout' runs.",
    )
    ap.add_argument("--suite", default="tau2-bench", help="Benchmark name for the card.")
    ...
    if args.variants:
        ...
        for part in args.variants.split(","):
            ...
            if not part:
                continue
            if "=" not in part:
                raise SystemExit(f"--variants entry {part!r} is not 'split_name=run_tag'")
            nm, tag = part.split("=", 1)
            variant_specs.append((nm.strip(), tag.strip()))
    else:
        variant_specs = [
            (f"{sc}_{ck}", f"{run_tag_prefix}_{sc}_{ck}_heldout") for sc, ck in VARIANTS
        ]

    all_ok = True
    hparams: dict[str, Any] = {}
    manifest: list[dict[str, Any]] = []
    for name, run_tag in variant_specs:
        # scorer/em_checkpoint become row columns; derive them from the split name
        # ("policy_best" -> policy/best) so custom variants stay self-describing.
        scorer, _, ckpt = name.partition("_")
        ckpt = ckpt or "best"
        gen_file = _find_run(logs_root, env_dir, args.model_cfg, run_tag)
        hp = _run_hparams(gen_file)
        if hparams and hp["length_penalty"] != hparams["length_penalty"]:
            raise SystemExit(
                f"variants disagree on length_penalty ({hparams['length_penalty']} vs "
                f"{hp['length_penalty']}); they cannot share one dataset card."
            )
        hparams = hparams or hp
    ...
    lp = hparams.get("length_penalty") or 0.0
    formula = (
        "reward(z) = p(x | s, z)"
        if not lp
        else f"reward(z) = p(x | s, z) - {lp} * len_frac"
    )
    split_cfg = "\n".join(
        f"  - split: {nm}\n    path: {nm}.jsonl" for nm, _ in variant_specs
    )
    vtable = "\n".join(
        ["| file / split | scorer | em_checkpoint | relabel run |", "|---|---|---|---|"]
        + [
            f"| `{nm}.jsonl` | {nm.partition('_')[0]} | {nm.partition('_')[2] or 'best'} | `{tag}` |"
            for nm, tag in variant_specs
        ]
    )
    _em = hparams.get("em") or {}
    em_prov = (
        "\n".join(
            f"| `{k}` | `{_em[k]}` |"
            for k in ("optimizer", "learning_rate", "num_batches", "advantage_mode",
                      "length_penalty", "lora_config")
            if _em.get(k) is not None
        )
    )
    em_prov = ("| EM setting | value |\n|---|---|\n" + em_prov) if em_prov else (
        "_(EM run config not found alongside these logs.)_"
    )
    card = DATASET_CARD
    for token, val in {
        "%%DOMAIN%%": domain,
        ...
        "%%VARIANT_TABLE%%": vtable,
        "%%REWARD_FORMULA%%": formula,
        "%%EM_PROVENANCE%%": em_prov,
        "%%GROUP_SIZE%%": str(args.group_size),
        "%%LENGTH_PENALTY%%": str(lp),
        "%%REPO%%": args.repo,
    ...
    (out_dir / "export_manifest.json").write_text(
        json.dumps({"hparams": hparams, "variants": manifest}, indent=2)
    )
